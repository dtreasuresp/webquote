  ...
  if (quotations.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-xs text-[#555555] mt-4 font-mono">
          Debug: {quotations.length} quotations, {snapshots.length} snapshots
        </p>
      </div>
    )
  }
      ...
      {quotations.map((quotation) => {
        ...
        if (quotationSnapshots.length > 0) {
          console.log(`📋 Cotización ${quotation.id}:`, {
            totalSnapshots: quotationSnapshots.length,
            snapshotsActivos: quotationSnapshots.filter(s => s.activo).length,
            snapshotsSample: quotationSnapshots[0],
          })
        }
                      ...
                      <div className="pl-4 space-y-1">
                        {quotationSnapshots.length > 0 ? (
                          quotationSnapshots.map((snapshot, idx) => {
                            // Calcular versión: La versión actual es versionNumber
                            // Cada snapshot anterior es una versión menos
                            // Pero nunca puede ser menor a 1
                            const versionNum = Math.max(1, quotation.versionNumber - idx)
                            return (
                              <div
                                key={snapshot.id}
                          ...
                          paquetesConfigurados.map((snapshot) => (
                            ...
                            >
                              ...
                              <div className="text-[#888888] mt-1">
                                <div>
                                  Desarrollo: ${snapshot.costos?.inicial?.toLocaleString('es-CO') || 0}
                                </div>
                                <div>Año 1: ${snapshot.costos?.año1?.toLocaleString('es-CO') || 0}</div>
                              </div>
                            </div>
                          ))
                        ) : (
